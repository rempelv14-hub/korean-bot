...
async def send_first_message(message: Message):
    try:
        ...
        logging.error(f"Ошибка при отправке 1-го сообщения: {e}")
...
async def send_second_message(message: Message):
    try:
        ...
        logging.info(f"[{message.from_user.id}] Отправлено 2-е сообщение")
    except Exception as e:
        logging.error(f"Ошибка при отправке 2-го сообщения: {e}")
...
async def send_third_message(message: Message):
    try:
        ...
        logging.info(f"[{message.from_user.id}] Отправлено 3-е сообщение")
    except Exception as e:
        logging.error(f"Ошибка при отправке 3-го сообщения: {e}")
    ...
    async def chain():
        try:
            if not active_users[user_id]["paid"]:
                # 1 сообщение — сразу при старте бота
                await send_first_message(message)

            # 2 сообщение — сразу после нажатия старт
            ...
            # 4 сообщение — через 10 минут после 3-го

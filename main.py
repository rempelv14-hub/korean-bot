...
async def send_sixth(message: Message):
    ...
    logging.info(f"[{message.from_user.id}] Отправлено 6-е сообщение (1 кнопка)")
...
def start_message_chain(user_id: int, message: Message):
    async def chain():
        ...
        # 3 сообщение — PDF через 5 минут
        await asyncio.sleep(5 * 60)
        await send_pdf(message)

        # 4 сообщение — через 10 минут после PDF
    ...
    active_users[user_id] = {"tasks": [task]}

    ...
    async def chain():
        try:
            if not active_users[user_id]["paid"]:
                await send_video(message)  # 1 сообщение сразу
            await asyncio.sleep(5 * 60)
            if not active_users[user_id]["paid"]:
                await send_pdf(message)  # 2 сообщение через 5 минут
            await asyncio.sleep(5 * 60)
            if not active_users[user_id]["paid"]:
                await send_course_presentation(message)  # 3 сообщение через 10 минут после старта
            await asyncio.sleep(5 * 60)
            if not active_users[user_id]["paid"]:
                await send_useful_tips(message)  # 4 сообщение через 15 минут после старта
            await asyncio.sleep(3 * 60 * 60)
            if not active_users[user_id]["paid"]:
                await send_final_message(message)  # 5 сообщение через 3 часа
            await asyncio.sleep(3 * 24 * 60 * 60)
            if not active_users[user_id]["paid"]:
                await send_reminder(message)  # 6 сообщение через 3 дня
...
async def start_course(callback: CallbackQuery):
    ...
    await callback.answer()  # просто подтверждаем нажатие

...
from aiogram.client.exceptions import TelegramBadRequest, TelegramForbidden, TelegramRetryAfter
...
if not BOT_TOKEN:
    ...
    BOT_TOKEN = None  # чтобы код не падал
...
# ================== СООБЩЕНИЯ ==================
async def send_message_safe(func, message: Message):
    """Отправка сообщений с защитой от ошибок Telegram"""
    try:
        await func(message)
    except (TelegramBadRequest, TelegramForbidden, TelegramRetryAfter, Exception) as e:
        logging.error(f"Ошибка при отправке сообщения пользователю {message.from_user.id}: {e}")
...
def schedule_chain(user_id: int, message: Message):
    ...
    for func, delta in message_chain:
        job = scheduler.add_job(send_message_safe, DateTrigger(now + delta), args=[func, message])
        jobs.append(job)
...
async def start(message: Message):
    ...
    await send_message_safe(lambda m: m.answer(
        "안녕하세요!\n"
...
async def start_bot():
    scheduler.start()
    if bot:  # проверка, чтобы бот не падал без токена
        await dp.start_polling(bot)
    else:
        logging.error("Не удалось создать Bot: Token is invalid!")
